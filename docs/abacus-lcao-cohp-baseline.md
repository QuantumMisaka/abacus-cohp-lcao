...
本项目当前 Python 后处理脚本位于 `src/cohp.py`，ABACUS 输出解析工具位于 `src/read_abacus_out.py`。`src/` 目录即本项目的核心 COHP 计算代码目录：`read_abacus_out.py` 负责把 ABACUS LCAO SCF 的矩阵、波函数、k 点权重和费米能级读入 Python，`cohp.py` 在此基础上选择目标轨道集合、计算 COHP/COOP，并输出曲线数据和图片。
...
该命令输出 `pair_COHP.dat` 和 `pair_COHP.png`。其中 `.dat` 是能量分辨 COHP 数据，`.png` 是绘图结果；`--invert` 用于按常见习惯绘制 `-COHP`。
